...
void simulateStep(double deltat,int rank, int size) {
    int n = N_BODIES/size;
...
int main(int argc, char *argv[]) {
    ...
    MPI_Gather(velx, n, MPI_DOUBLE, sub_velx, n, MPI_DOUBLE, 2,MPI_COMM_WORLD);
    MPI_Gather(vely, n, MPI_DOUBLE, sub_vely, n, MPI_DOUBLE, 2,MPI_COMM_WORLD);
    if (rank == 2) {      
